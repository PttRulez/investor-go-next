...
type ISSecurityInfo struct {
	Board     model.ISSMoexBoard
	Engine    model.ISSMoexEngine
	Market    model.ISSMoexMarket
	Name      string
	ShortName string
}
...
func (api *IssApiService) GetSecurityInfoBySecid(secid string) (*ISSecurityInfo, error) {
	...
	data := &model.MoexApiResponseSecurityInfo{}
	...
	board = model.ISSMoexBoard(boardData[0].(string))
	market = model.ISSMoexMarket(boardData[1].(string))
	engine = model.ISSMoexEngine(boardData[2].(string))
...
func (api *IssApiService) GetStocksCurrentPrices(ctx context.Context, market model.ISSMoexMarket,
	...
	data := &model.MoexApiResponseCurrentPrices{}
	err = json.Unmarshal(body, &data)
	if err != nil {
